...
עדכון אחרון: 2026-06-11 ערב — סיום בנייה ראשונה של ה-mockup המלא.
...
- [ ] לפתוח כל אחד מ-11 המסכים ולהשוות לצילום המקור ב-`.figma-refs/`
- [ ] לוודא ש**ה-RTL מתאים** — היה דיווח אחד שלא הוברר במלואו, יש לבחון שוב על מסכים שונים
- [ ] לבדוק שהאיור באמת מציג את הצורות הצבעוניות ולא רק רקע ריק (התיקון של ה-`cover` + `left center`)
...
6. **GitHub Pages** — האם להפעיל? `Settings → Pages → Branch: main → / (root)` ייצור URL נגיש (כמו `https://mimran2207-epr.github.io/Department-of-Education/`).
...
- **ה-PNGs כוללים את הצד הימני הריק של הפריים המקורי** — האיור מוקטן בעקבות זה. אם רוצים תצוגה חדה יותר, יש לחתוך את ה-PNGs ל-LEFT 44% או להוריד את ה-Figma exports בנפרד.
- **לוגו muni הוא חלק מה-PNG** ולא אלמנט HTML נפרד. אם החלפת PNG, ייתכן שהלוגו יעלם.
...
1. **לפתוח את הפרויקט** מ-VS Code או דפדפן: `pages/01-child-details.html`
2. **לבדוק את כל 11 המסכים** ולתעד בעיות (סעיף 3 לעיל)
3. **לפי הצורך:**
...
*מסמך זה עודכן בסיום העבודה ב-2026-06-11. ממשיכים מסעיף 3 בערב הבא.*
